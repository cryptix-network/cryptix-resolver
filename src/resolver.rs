...
impl Inner {
    fn new(args: &Arc<Args>) -> Self {
        Self {
            ...
            cryptix: Arc::new(Monitor::new(args, Service::Cryptix)),
...
impl Resolver {
    ...
    pub async fn init_http_server(self: &Arc<Self>) -> Result<()> {
        ...
        let this = self.clone();
        router = router.route(
            "/v2/cryptix/:network/:tls/:protocol/:encoding",
            get(|path| async move { this.get_elected_cryptix(path).await }),
        );
    ...
    pub async fn start(self: &Arc<Self>) -> Result<()> {
        self.inner.cryptix.start().await?;
    ...
    async fn update_nodes(self: &Arc<Self>, mut global_node_list: Vec<Arc<Node>>) -> Result<()> {
        self.inner.cryptix.update_nodes(&mut global_node_list).await?;
        self.inner
            .sparkle
    ...
    pub fn connections(&self) -> Vec<Arc<Connection>> {
        let cryptix = self.inner.cryptix.to_vec();

        let sparkle = self.inner.sparkle.to_vec();

        cryptix.into_iter().chain(sparkle).collect::<Vec<_>>()
    }

    // respond with a JSON object containing the elected node
    async fn get_elected_cryptix(
        &self,
        ...
        if let Some(json) = self.inner.cryptix.election(&params) {
